...
`Core::create_task` validates the source through `ResolverRegistry`, creates a `DownloadTask`, and writes it to the injected repository. `queue_task` adds it to the scheduler. The server's `task.start` selects the highest-priority queued HTTP/HTTPS task, claims a scheduler slot, resets its progress, clears the previous transfer error, and spawns an HTTP worker outside the Core mutex. The RPC returns the task ID before the transfer finishes. Magnet and local-file tasks remain queued because no transfer engine supports them; they do not block compatible HTTP/HTTPS tasks behind them. `Core::start_next` remains an in-memory-engine default for library callers; `start_next_with_engine("http")` is also a library API.
