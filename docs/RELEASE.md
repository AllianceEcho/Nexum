...
7. Smoke-test the binaries and native bundle against a local server. The current server keeps task state in memory and its `task start` RPC uses an in-memory engine. The browser extension's HTTP `/jsonrpc` transport is not implemented by that server; do not treat extension task creation as a passing release check yet.
