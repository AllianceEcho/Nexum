...
7. Smoke-test the binaries and native bundle against a local server. Serve a known file over HTTP, queue its URL, call `task start`, then query `task.get` or `task.list` while the worker is blocked to confirm an intermediate byte count, and check that the destination bytes and persisted `Completed` state match after the worker finishes. Verify that a failed transfer preserves any existing destination, exposes its message through the task `error` field, and retains that message after a restart with the same `--data-dir`; an interrupted `Downloading` task returns to `Queued`, and another `task start` begins the transfer from byte zero while clearing the previous error. Check that an active HTTP transfer rejects pause, resume, and remove, rejects data-directory and overlapping destinations. The browser extension's HTTP `/jsonrpc` transport is not implemented by that server; do not treat extension task creation as a passing release check yet.
